...
void FixHeatGran::post_create()
{
  ...
  //register heatFluxDistribution as property/atom  
  fix_heatFluxDistribution = static_cast<FixPropertyAtom*>(modify->find_fix_property("heatFluxDistribution","property/atom","vector",5,0,this->style,false));
  if (!fix_heatFluxDistribution)
  {
    const char* fixarg[13];
    fixarg[0] = "heatFluxDistribution";
    fixarg[1] = "all";
    fixarg[2] = "property/atom";
    fixarg[3] = "heatFluxDistribution";
    fixarg[4] = "vector"; 
    fixarg[5] = "no";  //restart or not?  
    fixarg[6] = "yes";    
    fixarg[7] = "no";    
    fixarg[8] = "0.";   
    fixarg[9] = "0."; 
    ...
    fixarg[11] = "0."; 
    fixarg[12] = "0.";  
    modify->add_fix(13,const_cast<char**>(fixarg));
    fix_heatFluxDistribution = static_cast<FixPropertyAtom*>(modify->find_fix_property("heatFluxDistribution","property/atom","vector",5,0,style));
  }
}
...
void FixHeatGran::updatePtrs(){
  ...
  heatFluxDistribution = fix_heatFluxDistribution->array_atom; // update heatFluxDistribution variable
}
...
void FixHeatGran::initial_integrate(int vflag)
{
  ...
  for (int i = 0; i < nlocal; i++)
  {
     if (mask[i] & groupbit)
     {
        ...
        heatFluxDistribution[i][0] = 0.; // initializing to zero
        heatFluxDistribution[i][1] = 0.;
        heatFluxDistribution[i][2] = 0.;
        heatFluxDistribution[i][3] = 0.;
        heatFluxDistribution[i][4] = 0.;
 ...
 // fix_directionalHeatFlux->do_forward_comm();
  //fix_heatFluxDistribution->do_forward_comm();
}
