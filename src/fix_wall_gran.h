...
#include "contact_interface.h" //added by Fenglei Qi
...
class FixWallGran : public Fix, public LIGGGHTS::IContactHistorySetup {
 ...
 protected:
  ...
  void addHeatFlux(class TriMesh *mesh,int i,LCM::SurfacesIntersectData & sidata,double area_ratio);
  void addNoncontactingHeatFlux(TriMesh *mesh,int ip, LCM::SurfacesCloseData const *scdata, double area_ratio);
  ...
  class FixPropertyAtom *fppa_hf_dist; //heat flux distribution
  const double *th_condf; //fluid conductivity 

  //adding radiation emissivity
  const double *emissivity;
  //adding struct data type
  struct pfp_param //particle-fluid-particle pathsway(pfp)
  {
    double lowBound,upBound;
    double condi,condw,condf;
    double radi;
    double rad_eff; //radius of pfp heat transfer area
    double gap;

    pfp_param():
                lowBound(0.0),upBound(0.0),
                condi(0.0),condw(0.0),condf(0.0),
                radi (0.0),
                rad_eff(0.0),
                gap(0.0) {}
  };
  ...
  //pfp functions, added by Fenglei Qi
  inline double pfp_heat_transfer(const FixWallGran::pfp_param &pp, const double &rr);
  typedef double (FixWallGran::*hfun)(const FixWallGran::pfp_param &pp, const double &rr);
  double num_integrate(FixWallGran::hfun fun,const FixWallGran::pfp_param &pfp,int numIntervals);
  double viewfactor_correlation_contacted_sphere(const FixWallGran::pfp_param &pfp); //added by Fenglei QI, // 2016/02/22
  double viewfactor_correlation_contacted_wall(const FixWallGran::pfp_param &pfp);
  double viewfactor_correlation_uncontacted_sphere(const FixWallGran::pfp_param &pfp);
  double viewfactor_correlation_uncontacted_wall(const FixWallGran::pfp_param &pfp);
